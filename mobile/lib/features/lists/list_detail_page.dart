...
class ListDetailPage extends StatefulWidget {
  const ListDetailPage(
      {required this.apiClient, required this.listId, super.key});
...
class _ListDetailPageState extends State<ListDetailPage> {
  ...
  Future<void> _toggleItem(ShoppingListItem item, bool? checked) async {
    ...
    try {
      await widget.apiClient.updateItem(
          widget.listId, item.id, item.toDraft().copyWith(isChecked: checked));
      await _reloadItems(silent: true);
    } catch (error) {
      if (!mounted) {
        return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Nie udało się zmienić stanu pozycji: $error')));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(title: Text('Lista ${widget.listId}'), actions: [
          IconButton(
              onPressed: () => _reloadItems(), icon: const Icon(Icons.refresh))
        ]),
          ...
          children: [
            ...
            Padding(
                ...
                child: Column(children: [
                  ...
                  Text('Nie udało się pobrać pozycji',
...
class _ItemEditorDialogState extends State<_ItemEditorDialog> {
  ...
  void _submit() {
    ...
    Navigator.of(context).pop(ItemDraft(
